...
# First, try to load default .env to get ENVIRONMENT variable
...
# Now load the appropriate env file based on ENVIRONMENT
if ENVIRONMENT == 'dev-local':
    ...
    if os.path.exists(load_path):
        load_dotenv(dotenv_path=load_path, override=True)
elif ENVIRONMENT == 'selfhost':
    dotenv_path = os.path.join(server_root, '.env.selfhost')
    if os.path.exists(dotenv_path):
        load_dotenv(dotenv_path=dotenv_path, override=True)
    else:
        # Fallback to .env if .env.selfhost doesn't exist
