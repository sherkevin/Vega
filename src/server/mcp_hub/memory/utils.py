...
from main.config import EMBEDDING_MODEL_NAME, GEMINI_API_KEY, OPENAI_API_BASE_URL, OPENAI_API_KEY

logger = get_logger(__name__)

# --- Module-level state (initialized by lifespan event) ---
embed_model_name: str = None
embedding_client = None  # OpenAI client for OpenAI-compatible APIs
use_openai_compatible = os.getenv("USE_OPENAI_COMPATIBLE_EMBEDDING", "false").lower() == "true"
agents: Dict[str, Any] = {}

# --- Initialization Functions ---
def initialize_embedding_model():
    global embed_model_name, embedding_client
    if embed_model_name is None:
        if use_openai_compatible:
            # Use OpenAI-compatible API (e.g., GLM-Embedding-3)
            if not OPENAI_API_KEY:
                raise ValueError("OPENAI_API_KEY is not set in main.config.")
            if not OPENAI_API_BASE_URL:
                raise ValueError("OPENAI_API_BASE_URL is not set in main.config.")
            logger.info(f"Initializing OpenAI-compatible embedding model: {EMBEDDING_MODEL_NAME} at {OPENAI_API_BASE_URL}")
            embedding_client = OpenAI(api_key=OPENAI_API_KEY, base_url=OPENAI_API_BASE_URL)
            ...
            logger.info(f"Initializing Gemini embedding model: {EMBEDDING_MODEL_NAME}")
...
def _get_normalized_embedding(text: str, task_type: str) -> np.ndarray:
    ...
    if use_openai_compatible:
        ...
        embedding_np = np.array(response.data[0].embedding, dtype=np.float32)
    else:
